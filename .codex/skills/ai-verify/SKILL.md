...
2. **Dependency preflight** — verify `.codex/skills/ai-verify/handlers/verify.md` plus required `.codex/agents/internal/verifier-*.md` files exist for the selected mode (`normal` and `--full` both require deterministic + acceptance post-W3; individual modes require only the matching specialist). STOP and report exact missing path(s) — never improvise.
